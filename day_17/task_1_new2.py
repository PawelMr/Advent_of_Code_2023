...
def find_possible_directions(point, direction, steps_taken, m, n):
    ...
    for options in all_options:
        if step_back != options:
            start_taken = 1 if step_forward != options else steps_taken + 1
            for i in range(start_taken, 4):
                y = options[0] * i
                x = options[1] * i
...
def get_new_point(ful_map, dict_min_sum_in_point):
        ...
        for y in range(m):
            for x in range(n):
                ...
                for variant in variants:
                    list_next_step = find_possible_directions(key_1, *variant, m, n)
